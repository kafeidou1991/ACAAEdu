...
//    4）未通过。  #ED2323
    ...
    if (item.status.intValue == 0) { //未考试
        ...
        self.statusView.backgroundColor = self.examRightView.tintColor = AEHexColor(@"B778FF");
    } else { //完成考试
        self.resultLabel.text = [NSString stringWithFormat:@"考试结果:%@",item.pass == 1 ? @"通过" : @"未通过"];
        self.examStatusLabel.text = @"考试结束";
        self.statusView.backgroundColor = self.examRightView.tintColor = AEHexColor(item.pass == 1 ? @"4ED3C1" : @"ED2323");
    }
}
