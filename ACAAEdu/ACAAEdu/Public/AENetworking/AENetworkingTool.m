...
+ (instancetype)sharedManager {
    ...
    static AENetworkingTool *instance;
    dispatch_once(&onceToken, ^{
        NSURL *baseUrl = [NSURL URLWithString:@""];
        instance = [[AENetworkingTool alloc] initWithBaseURL:baseUrl];
        // https ssl 验证。
...
+ (void)GetWithUrl: (NSString *)url params: (NSDictionary *)params isReadCache: (BOOL)isReadCache success: (responseSuccess)success failed: (ResponseFailed)failed{
    ...
    [[AENetworkingTool sharedManager] GET:url parameters:tempDict progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"GET------%@\n参数%@\njson----------%@",url,tempDict,[[AENetworkingTool sharedManager]DataTOjsonString:responseObject]);
        NSError * error = [[AENetworkingTool sharedManager]checkIsSuccess:responseObject Url:url];
        if (error == nil) {
            if (success) {
                //成功的话 返回data里面的内容
            ...
            //请求成功,保存数据
            [AECache saveDataCache:responseObject forKey:url];
        }else{
            //用户需要重新登录
            if (error.code == -1) {
//                [[NSNotificationCenter defaultCenter] postNotificationName:ReplaceLogin object:nil];
        ...
        //是否读取缓存
        if (isReadCache) {
            cacheData = [AECache readCache:url];
        }else {
            cacheData = nil;
        }
        if (failed) {
...
+ (void)PostWithUrl:(NSString *)url params:(NSDictionary *)params isReadCache:(BOOL)isReadCache success:(responseSuccess)success failed:(ResponseFailed)failed{
    NSMutableDictionary * tempDict = params.mutableCopy;
    [[AENetworkingTool sharedManager] POST:url parameters:tempDict progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"POST------%@\n参数%@\njson----------%@",url,tempDict,[[AENetworkingTool sharedManager]DataTOjsonString:responseObject]);
        NSError * error = [[AENetworkingTool sharedManager]checkIsSuccess:responseObject Url:url];
        if (error == nil) {
            if (success) {
...
+ (void)uploadWithUrl: (NSString *)url params: (NSDictionary *)params fileData: (NSData *)fileData name: (NSString *)name fileName: (NSString *)fileName mimeType: (NSString *)mimeType progress: (progress)progress success: (responseSuccess)success failed: (responseFailed)failed {
    [[AENetworkingTool sharedManager] POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (void)downloadWithUrl: (NSString *)url {
    ...
    // 2. 发起下载任务
    [AENetworkingTool sharedManager].downloadTask = [[AENetworkingTool sharedManager].downloadSession downloadTaskWithURL:URL];
    
    // 3. 启动下载任务
    [[AENetworkingTool sharedManager].downloadTask resume];
